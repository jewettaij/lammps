  ...
  probability(NULL), created(NULL), copy(NULL), random(NULL), list(NULL)
{
  ...
  if (cutoff < 0.0) error->all(FLERR,"Illegal fix bond/new command");
  if (btype < 1 || btype > atom->nbondtypes)
    error->all(FLERR,"Invalid bond type in fix bond/new command");
  ...
  if (atom->molecular != 1)
    error->all(FLERR,"Cannot use fix bond/new with non-molecular systems");
  if (iatomtype == jatomtype &&
      ((imaxbond != jmaxbond) || (inewtype != jnewtype) ||
       (inewtype_override != jnewtype_override)))
    error->all(FLERR,
               "Inconsistent iparam/jparam values in fix bond/new command");
...
void FixBondNew::init()
{
  ...
  //AJMOD remove this useless check!  When using Lennard Jones forces, it makes it 
  //impossible to form bonds between two particles that are otherwise repulsive
  //if (force->pair == NULL || cutsq > force->pair->cutsq[iatomtype][jatomtype])
  //  error->all(FLERR,"Fix bond/new cutoff is longer than pairwise cutoff");
  ...
  if (atype && force->angle) {
    ...
    if (atype > atom->nangletypes)
      error->all(FLERR,"Fix bond/new angle type is invalid");
  } else angleflag = 0;

  if (dtype && force->dihedral) {
    dihedralflag = 1;
    if (dtype > atom->ndihedraltypes)
      error->all(FLERR,"Fix bond/new dihedral type is invalid");
  } else dihedralflag = 0;

  if (itype && force->improper) {
    improperflag = 1;
    if (itype > atom->nimpropertypes)
      error->all(FLERR,"Fix bond/new improper type is invalid");
  } else improperflag = 0;

  if (force->improper) {
    if (force->improper_match("class2") || force->improper_match("ring"))
      error->all(FLERR,"Cannot yet use fix bond/new with this "
                 "improper style");
  }
...
void FixBondNew::init_list(int id, NeighList *ptr)
{
  list = ptr;
}
...
void FixBondNew::setup(int vflag)
{
  int i,j,m;
    ...
    for (j = 0; j < num_bond[i]; j++) {
      if (bond_type[i][j] == btype) {
        ...
        if (newton_bond) {
          ...
          if (m < 0)
            error->one(FLERR,"Fix bond/new needs ghost atoms "
                       "from further away");
          bondcount[m]++;
        }
      }
...
void FixBondNew::post_integrate()
{
  ...
  if (atom->nmax > nmax) {
    ...
    memory->destroy(distsq);
    nmax = atom->nmax;
    memory->create(partner,nmax,"bond/new:partner");
    memory->create(finalpartner,nmax,"bond/new:finalpartner");
    memory->create(distsq,nmax,"bond/new:distsq");
    probability = distsq;
  }
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    //char dbg_msg[BUFFLEN];
    //snprintf(dbg_msg, BUFFLEN,
    //         "%s[%s], t=%d: atom->type[atomid=" TAGINT_FORMAT "] = %d, mpirank=%d\n",
    ...
    //if ((atom->type[i] == 7) || (atom->type[i] == 8)) {  // for DEBUGGING only.  remove
    //  inum++;
    //  inum--;
    //}
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      //if ((atom->type[j] == 10) || (atom->type[j] == 11)) {  // for DEBUGGING only.  remove
      //  jnum++;
      //  jnum--;
      //}
      ...
      if (itype == iatomtype && jtype == jatomtype) {
        if (((imaxbond == 0 || bondcount[i] < imaxbond) &&
            ...
            ((inewtype_override > 0) || (jnewtype_override > 0))) //AJMOD
          possible = 1;
      ...
      //if ((update->ntimestep >= 700) && (tag[ii]==3) && (strcmp(this->id,"a1a")==0)) {
      //static const int BUFFLEN=2048;
      //char dbg_msg[BUFFLEN];
      //snprintf(dbg_msg, BUFFLEN,
      //         "%s[%s], t=%d: possible bond between (atomid=" TAGINT_FORMAT ",atomid=" TAGINT_FORMAT "), mpirank=%d\n",
      //         this->style, this->id, update->ntimestep, 
      //         tag[i], tag[j], me);
      ////error->message(FLERR, dbg_msg);
      //fprintf(stderr, dbg_msg);
  ...
  for (i = 0; i < nlocal; i++) {
    ...
    //if (!newton_bond || tag[i] < tag[j]) {   <--AJMOD
      if (num_bond[i] == atom->bond_per_atom)
        error->one(FLERR,"New bond exceeded bonds per atom in fix bond/new");
      static const int BUFFLEN=2048;
      char dbg_msg[BUFFLEN];
      snprintf(dbg_msg, BUFFLEN,
               "%s[%s], t=%d: new_bond[%d](atomid=" TAGINT_FORMAT
               ",atomid=" TAGINT_FORMAT ") = %d, mpirank=%d\n",
               this->style, this->id, update->ntimestep, 
               num_bond[i], tag[i], tag[j], btype, me);
      //error->message(FLERR, dbg_msg);
      fprintf(stderr, dbg_msg);
    ...
    if (n3 == atom->maxspecial)
      error->one(FLERR,
                 "New bond exceeded special list size in fix bond/new");
    ...
    //if (((inewtype_override>0) || (jnewtype_override>0)) && (tag[i]<tag[j])) { //AJMOD
    if ((inewtype_override>0) || (jnewtype_override>0)) { //AJMOD
      #ifndef NDEBUG
      int itype_old = type[i];
      int jtype_old = type[j];
      #endif
      if ((type[i] == iatomtype) && (type[j] == jatomtype)) {
        if (inewtype_override > 0)
          type[i] = inewtype_override;
        if (jnewtype_override > 0)
          type[j] = jnewtype_override;
      }
      else if ((type[i] == jatomtype) && (type[j] == iatomtype)) {
        if (inewtype_override > 0)
          type[j] = inewtype_override;
        if (jnewtype_override > 0)
          type[i] = jnewtype_override;
      }
      #ifndef NDEBUG
      if ((type[i] != itype_old) || (type[j] != jtype_old)) {
        static const int BUFFLEN=2048;
        char dbg_msg[BUFFLEN];
        snprintf(dbg_msg, BUFFLEN,
                 "%s[%s], t=%d: atoms(type["TAGINT_FORMAT"]=%d,type["TAGINT_FORMAT"]=%d) -> atoms(type=%d,type=%d), mpirank=%d\n",
                 this->style, this->id, update->ntimestep, 
                 tag[i], itype_old, tag[j], jtype_old, type[i], type[j], me);
        //error->message(FLERR, dbg_msg);
        fprintf(stderr, dbg_msg);
      }
      #endif
  ...
  for (i = 0; i < nall; i++) {
    ...
    if (j < 0 || tag[i] < tag[j]) {
      if (ncreate == maxcreate) {
        maxcreate += DELTA;
        memory->grow(created,maxcreate,2,"bond/new:created");
      }
...
void FixBondNew::check_ghosts()
{
  ...
  if (flagall)
    error->all(FLERR,"Fix bond/new needs ghost atoms from further away");
...
void FixBondNew::update_topology()
{
  ...
  if (overflowall) error->all(FLERR,"Fix bond/new induced too many "
...
void FixBondNew::rebuild_special_one(int m)
{
  ...
  for (i = 0; i < cn1; i++) {
    n = atom->map(copy[i]);
    if (n < 0)
      error->one(FLERR,"Fix bond/new needs ghost atoms from further away");
    slist = special[n];
    n1 = nspecial[n][0];
    for (j = 0; j < n1; j++)
  ...
  if (cn2 > atom->maxspecial)
    error->one(FLERR,"Special list size exceeded in fix bond/new");
...
void FixBondNew::create_angles(int m)
{
...
void FixBondNew::create_dihedrals(int m)
{
...
void FixBondNew::create_impropers(int m)
{
...
int FixBondNew::dedup(int nstart, int nstop, tagint *copy)
{
  int i;
...
void FixBondNew::post_integrate_respa(int ilevel, int iloop)
{
...
int FixBondNew::pack_forward_comm(int n, int *list, double *buf,
...
void FixBondNew::unpack_forward_comm(int n, int first, double *buf)
{
  int i,j,m,ns,last;
...
int FixBondNew::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixBondNew::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
void FixBondNew::grow_arrays(int nmax)
{
  memory->grow(bondcount,nmax,"bond/newy:bondcount");
}
...
void FixBondNew::copy_arrays(int i, int j, int delflag)
{
...
int FixBondNew::pack_exchange(int i, double *buf)
{
  buf[0] = bondcount[i];
  return 1;
}
...
int FixBondNew::unpack_exchange(int nlocal, double *buf)
{
...
double FixBondNew::compute_vector(int n)
{
...
double FixBondNew::memory_usage()
{
  int nmax = atom->nmax;
...
void FixBondNew::print_copy(const char *str, tagint m,
